...
static uint32_t _def_bg = 0x080808;
static uint32_t _def_fg = 0xFFFFFF;
...
void tty_enable() {
  ...
  if (!ftctx) {
    struct limine_framebuffer *f = fb_get_limine(0);

    ftctx = (struct flanterm_fb_context*)flanterm_fb_init(
        NULL, NULL, (uint32_t*)f->address, f->width, f->height, f->pitch,

        f->red_mask_size, f->red_mask_shift, f->green_mask_size,
        f->green_mask_shift, f->blue_mask_size, f->blue_mask_shift,

        NULL, NULL, NULL, &_def_bg, &_def_fg, NULL, NULL,

        &font, FONT_WIDTH, FONT_HEIGHT, 0, 0, 0, 5);
  }
